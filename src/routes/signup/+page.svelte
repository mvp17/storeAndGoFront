	...
	<form on:submit|preventDefault={signup} class="space-y-6">
	  <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
		<div>
		  ...
		  <Input type="text" id="firstName" placeholder="John" bind:value={firstName} class="w-full p-2 border rounded" required />
		</div>
		<div>
		  <Label for="lastName" class="mb-1">Last Name</Label>
		  <Input type="text" id="lastName" placeholder="Rambo" bind:value={lastName} class="w-full p-2 border rounded" required />
		</div>
	  </div>
	  <div>
	  ...
	  <div>
		...
		<Input type="email" id="email" placeholder="john.doe@company.com" bind:value={email} class="w-full p-2 border rounded" required />
	  </div>
	  <div>
	  ...
	  <div class="flex justify-between mt-6">
		<Button type="submit" class="w-full mr-2">Sign Up</Button>
